<template>
  <section class="account-page">
    ...
    <template v-else-if="session">
      ...
      <section class="work-panel account-card"><span class="status-pill">已登录</span><h2>{{ session.name }}</h2><p>{{ session.email || '第三方身份账户' }}</p><p>当前账户会话有效。不同邮箱的 GitHub 和 Microsoft 身份也可以绑定到同一账户。</p><div class="actions"><button class="secondary-button" :disabled="busy" @click="link('github')">绑定 GitHub</button><button class="secondary-button" :disabled="busy" @click="link('google')">绑定 Google</button><button class="secondary-button" :disabled="busy" @click="link('microsoft')">绑定 Microsoft</button></div></section>
...
function link(provider:'github'|'microsoft'|'google'){ busy.value=true; platform.oauthStart(provider, '/account', 'link'); }
